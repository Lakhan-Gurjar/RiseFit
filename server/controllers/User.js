...
export const UserRegister = async (req, res, next) => {
    try {
        ...
        const exisitingUser = await User.findOne({ email }).exec();
        if (exisitingUser) {
            return next((createError(409, "Email is already is in use")));
        }
...
export const UserLogin = async (req, res, next) => {
    try {
        ...
        if (!user) {
            return next((createError(40, "User not found")));
        }
            ...
            const weekData = await Workout.aggregate([
                ...
                },
                {
                    $group: {
                        ...
                        totalCaloriesBurnt: { $sun: "$caloriesBurned" },
                    },
                },
                {
